    ...
    def training_step(self, batch, batch_idx, jointist=None):
        ...
        if (self.current_epoch+1)%(self.trainer.check_val_every_n_epoch)==0: # log the label count only once        
            ...
            output_batch = {
                ...
                'multihot_pred': multihot_pred.detach(),
    ...
    def training_epoch_end(self, outputs, jointist=None):
        if (self.current_epoch+1)%(self.trainer.check_val_every_n_epoch)==0: # log the label count only once        
            ...
            for batch in outputs:
                pred = (torch.sigmoid(batch['src_logits'])>0.5).float()
                y_pred_map.append(pred)
                y_label_map.append(batch['instruments'])         
                
            y_pred_map = torch.cat(y_pred_map).flatten(1)
...
#             metrics = self.get_accuracy(y_label, y_logits, OmegaConf.to_object(self.IX_TO_NAME))
            ...
            if (self.current_epoch+1)==self.trainer.check_val_every_n_epoch: # log the label count only once
                _ = self.barplot(label_stat, 'train_label_counts (log_e)', (4,12), 0.2, log=True)
            _ = self.barplot(pred_stat, 'train_pred_counts (log_e)', (4,12), 0.2, log=True)
            logger.logger.experiment.add_figure(f"Train/F1 scores",
                                                self.barplot(f1_stat, 'F1', (4,12), 0.05, log=False),
                                                global_step=self.current_epoch)

            for key in metrics:
                for instrument in metrics[key]:
                    logger.log(f"{key}_Train/{instrument}", metrics[key][instrument], on_step=False, on_epoch=True, rank_zero_only=True)             
            for key in f1_dicts:
                if key!='none':
                    logger.log(f"F1_{key}_Train/", f1_dicts[key], on_step=False, on_epoch=True, rank_zero_only=True)
                else:
                    for idx, f1_score in enumerate(f1_dicts[key]):
                        instrument = self.IX_TO_NAME[idx]
                        logger.log(f"F1_Train/{instrument}", f1_dicts[key][idx], on_step=False, on_epoch=True, rank_zero_only=True)    
    ...
    def validation_epoch_end(self, outputs, jointist=None):
        ...
        y_pred_map = []
        y_label_map = []        
        for batch, _ in outputs:
            pred = (torch.sigmoid(batch['src_logits'])>0.5).float()
            y_pred_map.append(pred)
            y_label_map.append(batch['instruments'])            

        y_pred_map = torch.cat(y_pred_map).flatten(1) #(dataset_size, num_classes)
...
#         metrics = self.get_accuracy(y_label, y_logits, OmegaConf.to_object(self.IX_TO_NAME))
        ...
        for key in [None, 'micro', 'macro', 'weighted', 'samples']:
            _, _, f1, _ = precision_recall_fscore_support(y_label_map.cpu(), y_pred_map.cpu(), average=key, zero_division=0)
            # f1.shape = (dataset_size, num_classes)
            if key:
                f1_dicts[key] = f1
            else:
                f1_dicts['none'] = f1
        ...
        if (self.current_epoch+1)==self.trainer.check_val_every_n_epoch: # log the label count only once
            _ = self.barplot(label_stat, 'valid_label_counts (log_e)', (4,12), 0.2, log=True)
        _ = self.barplot(pred_stat, 'valid_pred_counts (log_e)', (4,12), 0.2, log=True)
...
#         for key in metrics:
#             for instrument in metrics[key]:
#                 logger.log(f"{key}_Valid/{instrument}", metrics[key][instrument], on_step=False, on_epoch=True, rank_zero_only=True)                
        for key in f1_dicts:
            if key!='none':
                logger.log(f"F1_{key}_Valid/", f1_dicts[key], on_step=False, on_epoch=True, rank_zero_only=True)
            else:
                for idx, f1_score in enumerate(f1_dicts[key]):
                    instrument = self.IX_TO_NAME[idx]
                    logger.log(f"F1_Valid/{instrument}", f1_dicts[key][idx], on_step=False, on_epoch=True, rank_zero_only=True)
    ...
    def test_epoch_end(self, outputs, jointist=None):
        ...
        y_label_map = []        
        for batch, _ in outputs:
            y_logits.append(batch['src_logits'])
            y_label.append(batch['target_classes'])
        ...
        _ = self.barplot(pred_stat, 'test_pred_counts (log_e)', (4,12), 0.2, log=True)
        _ = self.barplot(label_stat, 'test_label_counts (log_e)', (4,12), 0.2, log=True)
        ...
        for key in metrics:
            for instrument in metrics[key]:
                logger.log(f"{key}_Test/{instrument}", metrics[key][instrument], on_step=False, on_epoch=True, rank_zero_only=True)
                
        for key in f1_dicts:
            if key!='none':
                logger.log(f"F1_{key}_Test/", f1_dicts[key], on_step=False, on_epoch=True, rank_zero_only=True)
            else:
                for idx, f1_score in enumerate(f1_dicts[key]):
                    instrument = self.IX_TO_NAME[idx]
                    logger.log(f"F1_Test/{instrument}", f1_dicts[key][idx], on_step=False, on_epoch=True, rank_zero_only=True)
...
#         mask = src_logits.argmax(-1)!=(src_logits.shape[-1]-1)
        ...
        y_score = torch.softmax(src_logits[mask], -1).detach().cpu()
